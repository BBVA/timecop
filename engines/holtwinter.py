...
from .helpers import create_train_test, windows
...
def anomaly_holt(lista_datos, desv_mse=0):
    ...
    for slot in ventanas:
        if count != 0:
            stepwise_model = ExponentialSmoothing(
                training_data, seasonal_periods=1)
            fit_stepwise_model = stepwise_model.fit()
    ...
    for period in range(2, 50):
        ...
        stepwise_model = ExponentialSmoothing(
            df_train['valores'], seasonal_periods=12, trend='add', seasonal='add', )
        fit_stepwise_model = stepwise_model.fit()

        future_forecast_pred = fit_stepwise_model.forecast(
            len(df_test['valores']))
        mae_temp = mean_absolute_error(
            future_forecast_pred.values, df_test['valores'].values)
        if mae_temp < mae_period:
            best_period = period
            mae_period = mae_temp
        else:
            print("mae:" + str(mae_temp))
    print("######best mae is " + str(mae_period) +
          " with the period " + str(best_period))
    ...
    df_aler = pd.DataFrame(forecast_pred10, index=df.index,
    ...
    stepwise_model1 = ExponentialSmoothing(
        df['valores'], seasonal_periods=len(df['valores']), seasonal='add')
...
def forecast_holt(lista_datos, num_fut):
    ...
    stepwise_model = ExponentialSmoothing(
        df, seasonal_periods=len(df), seasonal='add')
    fit_stepwise_model = stepwise_model.fit()

    future_forecast_pred = fit_stepwise_model.forecast(num_fut)

    df_result = pd.DataFrame(
        {'puntos': future_forecast_pred.index, 'valores': future_forecast_pred.values})
