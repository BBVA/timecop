...
def anomaly_AutoArima(lista_datos, desv_mse=0):
    ...
    future_forecast_pred = stepwise_model.predict(
        n_periods=len(df_test['valores']))
    ...
    df_aler = pd.DataFrame(future_forecast_pred,
                           index=df_test.index, columns=['expected value'])
    ...
    testing_data = pd.DataFrame(
        future_forecast_pred, index=df_test.index, columns=['expected value'])
